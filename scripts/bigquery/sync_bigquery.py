# ------------------------------------
# scripts/bigquery/sync_bigquery.py
#
# Synchronisiert lokale Outputs nach BigQuery.
#
# Ziel:
# - Append-Mode: nimmt die aktuellen Parquet-Snapshots aus data/warehouse/raw
#   und haengt sie an die bestehenden RAW-Tabellen in BigQuery an.
# - Replace-Mode: baut die RAW-Tabellen komplett neu aus den History-CSV-Dateien
#   in data/processed (WRITE_TRUNCATE).
#
# Usage
# ------------------------------------
# PowerShell (Windows):
#   $env:GOOGLE_APPLICATION_CREDENTIALS="C:\path\to\service_account.json"
#   $env:BQ_PROJECT_ID="house-price-model"
#
#   # Append latest snapshots
#   python -m scripts.bigquery.sync_bigquery --mode append --dataset house_prices_raw
#
#   # Full rebuild from processed history
#   python -m scripts.bigquery.sync_bigquery --mode replace --dataset house_prices_raw
# ------------------------------------
from __future__ import annotations

import argparse
import logging
import os
from pathlib import Path
from typing import Optional

import pandas as pd
from google.cloud import bigquery


def setup_logging(debug: bool) -> logging.Logger:
    logger = logging.getLogger("bq-sync")
    logger.setLevel(logging.DEBUG if debug else logging.INFO)

    if not logger.handlers:
        ch = logging.StreamHandler()
        ch.setLevel(logging.DEBUG if debug else logging.INFO)
        fmt = logging.Formatter("%(asctime)s [%(levelname)s] %(name)s: %(message)s")
        ch.setFormatter(fmt)
        logger.addHandler(ch)

    return logger


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Sync local outputs to BigQuery (append or replace).")
    p.add_argument("--project", type=str, default=None, help="GCP project id (env: BQ_PROJECT_ID)")
    p.add_argument("--dataset", type=str, default="house_prices_raw")
    p.add_argument("--location", type=str, default="EU")
    p.add_argument("--mode", choices=["append", "replace"], default="append")
    p.add_argument("--warehouse-raw-dir", type=str, default="data/warehouse/raw")
    p.add_argument("--processed-dir", type=str, default="data/processed")
    p.add_argument("--debug", action="store_true")
    return p.parse_args()


def _ensure_dataset(client: bigquery.Client, dataset_id: str, location: Optional[str], logger: logging.Logger) -> None:
    try:
        client.get_dataset(dataset_id)
        return
    except Exception:
        pass

    ds = bigquery.Dataset(dataset_id)
    if location:
        ds.location = location
    client.create_dataset(ds)
    logger.info("Dataset erstellt: %s", dataset_id)


def _load_parquet(
    client: bigquery.Client,
    dataset: str,
    table: str,
    parquet_path: Path,
    write_disp: str,
    logger: logging.Logger,
) -> None:
    table_id = f"{client.project}.{dataset}.{table}"

    job_config = bigquery.LoadJobConfig(
        source_format=bigquery.SourceFormat.PARQUET,
        write_disposition=write_disp,
        schema_update_options=[bigquery.SchemaUpdateOption.ALLOW_FIELD_ADDITION],
    )

    with parquet_path.open("rb") as f:
        job = client.load_table_from_file(f, table_id, job_config=job_config)

    job.result()
    logger.info("Loaded %s -> %s", str(parquet_path.as_posix()), table_id)


def _replace_from_processed(
    client: bigquery.Client,
    dataset: str,
    processed_dir: Path,
    tmp_dir: Path,
    logger: logging.Logger,
) -> None:
    tmp_dir.mkdir(parents=True, exist_ok=True)

    mapping = {
        "models": processed_dir / "models.csv",
        "train_cv_predictions": processed_dir / "train_cv_predictions.csv",
        "predictions": processed_dir / "predictions.csv",
    }

    for table, csv_path in mapping.items():
        if not csv_path.exists():
            logger.info("Skip %s (Datei fehlt): %s", table, str(csv_path.as_posix()))
            continue

        df = pd.read_csv(csv_path, low_memory=False)

        if "created_at_utc" in df.columns:
            df["created_at_utc"] = pd.to_datetime(df["created_at_utc"], errors="coerce", utc=True)

        parquet_path = tmp_dir / f"{table}.parquet"
        df.to_parquet(parquet_path, index=False)

        _load_parquet(
            client=client,
            dataset=dataset,
            table=table,
            parquet_path=parquet_path,
            write_disp=bigquery.WriteDisposition.WRITE_TRUNCATE,
            logger=logger,
        )


def main() -> int:
    args = parse_args()
    logger = setup_logging(args.debug)

    project = args.project or os.environ.get("BQ_PROJECT_ID")
    if not project:
        raise ValueError("BQ project fehlt. Nutze --project oder env:BQ_PROJECT_ID.")

    client = bigquery.Client(project=project)

    dataset_id = f"{project}.{args.dataset}"
    _ensure_dataset(client, dataset_id, location=args.location, logger=logger)

    if args.mode == "append":
        raw_dir = Path(args.warehouse_raw_dir)
        mapping = {
            "models": raw_dir / "models.parquet",
            "train_cv_predictions": raw_dir / "train_cv_predictions.parquet",
            "predictions": raw_dir / "predictions.parquet",
        }

        for table, path in mapping.items():
            if not path.exists():
                logger.info("Skip %s (Datei fehlt): %s", table, str(path.as_posix()))
                continue
            _load_parquet(
                client=client,
                dataset=args.dataset,
                table=table,
                parquet_path=path,
                write_disp=bigquery.WriteDisposition.WRITE_APPEND,
                logger=logger,
            )
        return 0

    processed_dir = Path(args.processed_dir)
    tmp_dir = processed_dir / ".tmp_bq_upload"
    _replace_from_processed(client, args.dataset, processed_dir, tmp_dir, logger)
    return 0


if __name__ == "__main__":
    raise SystemExit(main())